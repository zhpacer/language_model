...
class NgramTrain:
    def __init__(self,ngram_count,args):
        ...
        self.default_backoff_prob = -1000
    ...
    def CalcBackoffProb(self):
        for order in range(self.ngram_order):
            ...
            for ngram in self.ngram_prob[order].keys():
                ...
                backoff_prob = self.default_backoff_prob
                if base_prob == 0:
                    backoff_prob = 1000
                else:
        ...
        for order in range(self.ngram_order):
            ...
            for (ngram,count) in self.ngram_count[order].items():
                if order == 1:
                    ...
                    self.ngram_prob[order][ngram]["backoff"]=self.default_backoff_prob
                else:
    ...
    def WriteArapa(self,prob_file):
       ...
       for order in range(self.ngram_order): 
            ...
            for word in sorted(self.ngram_prob[order].keys()):
                ...
                if backoff <= 0 and backoff != self.default_backoff_prob : 
...
if __name__ == '__main__':
    ...
    ngram_prob = NgramTrain(ngram_count.GetNgramCount(),args)
    ngram_prob.CalcProb()
