...
Typically $f(a_{n}|a_1\cdots a_{n-1})$ is discounted to be less than the ML estimate so we have some leftover probability for the $a_n$ words unseen in the context ( $a_1\cdots a_{n-1}$ ).    
...
$$\sum_{Z_1}p(a_n|a_1\cdots a_{n-1}) + \sum_{Z_0}bow(a_{n-1}|a_1\cdots a_{n-2}) \times p( a_{n}|a_2\cdots a_{n-1}) = 1$$
$$bow(a_1\cdots a_{n-1}) = \frac{1-\sum_{Z_1}f(a_n|a_1\cdots a_{n-1})} {\sum_{Z_0}p(a_{n}|a_2\cdots a_{n-1})}$$
$$= \frac{1-\sum_{Z_1}f(a_n|a_1\cdots a_{n-1})} {1-\sum_{Z_1}p(a_{n}|a_2\cdots a_{n-1})}$$
$$=  \frac{1-\sum_{Z_1}f(a_n|a_1\cdots a_{n-1})} {1-\sum_{Z_1}f(a_{n}|a_2\cdots a_{n-1})}$$
Smoothing is generally done in one of two ways. The backoff models compute $p(a_{n}|a_1\cdots a_{n-1})$ based on the N-gram counts $c(a_1\cdots a_{n})$ when $c(a_1\cdots a_{n}) > 0$, 
...
(4)&nbsp;&nbsp; $$p(a_1\cdots a_{n}) = g(a_1\cdots a_{n}) + bow(a_{n-1}|a_1\cdots a_{n-2}) * p(a_{n}|a_2\cdots a_{n-1})$$  
...
Given $g(a_1\cdots a_{n})$ , $bow(a_{n-1}|a_1\cdots a_{n-2})$ can be determined as follows:  
...
(6) $$f(a_n|a_1\cdots a_{n-1}) = bow(a_{n-1}|a_1\cdots a_{n-2}) * p(a_{n}|a_2\cdots a_{n-1})$$
$$p(a_n|a_1\cdots a_{n-1}) = (c(a_1\cdots a_{n}) > 0) ? f(a_n|a_1\cdots a_{n-1}) : bow(a_{n-1}|a_1\cdots a_{n-2}) * p(a_{n}|a_2\cdots a_{n-1})$$
...
After giving the motivation for each discounting method, we will give expressions for $f(a_n|a_1\cdots a_{n-1})$ and $bow(a_{n-1}|a_1\cdots a_{n-2})$  of Equation 2 in terms of the counts.   
...
In this case we will express $g(a_1\cdots a_{n})$ and $bow(a_{n-1}|a_1\cdots a_{n-2})$  of Equation 4 in terms of the counts as well.  
Note that the ARPA format model files store the interpolated models and the backoff models the same way using  $f(a_n|a_1\cdots a_{n-1})$  and $bow(a_{n-1}|a_1\cdots a_{n-2})$;  
...
-cdiscount D  
Ney's absolute discounting using D as the constant to subtract. D should be between 0 and 1. If $Z1$ is the set of all words $z$ with $c(a_1\cdots a_{n}) > 0$ :  
$$f(a_n|a_1\cdots a_{n-1}) = \frac{c(a_1\cdots a_{n-1}) - D } {c(a_1\cdots a_{n-1}) }$$
$$p(a_n|a_1\cdots a_{n-1}) = (c(a_1\cdots a_{n}) > 0) ? f(a_n|a_1\cdots a_{n-1}) : bow( a_{n-1}|a_1\cdots a_{n-2}) * p(a_{n}|a_2\cdots a_{n-1})\quad\quad Eqn.2 $$ 
$$bow(a_1\cdots a_{n-1})  = \frac{1-\sum_{Z_1}f(a_n|a_1\cdots a_{n-1})} {1-\sum_{Z_1}f(a_{n}|a_2\cdots a_{n-1})}\quad\quad Eqn.3  $$

With the -interpolate option we have:  
$$g(a_n|a_1\cdots a_{n-1}) = \frac{max(0,c(a_1\cdots a_{n}) - D) } {c(a_1\cdots a_{n-1}) }$$
$$p(a_n|a_1\cdots a_{n-1}) = g(a_n|a_1\cdots a_{n-1}) + bow(a_{n-1}|a_1\cdots a_{n-2}) * p(a_{n}|a_2\cdots a_{n-1})\quad\quad Eqn.4  $$
$$bow(a_{n-1}|a_1\cdots a_{n-2})  = 1-\sum_{Z_1}g(a_n|a_1\cdots a_{n-1})\quad\quad Eqn.5 $$
$$= \frac{D* n(a_1\cdots a_{n-1}*)} {c(a_1\cdots a_{n-2})}$$
...
$$f(a_n|a_1\cdots a_{n-1}) =  \frac{c(a_n|a_1\cdots a_{n-1}) - D_0 } {c(a_n-1|a_1\cdots a_{n-2}) }\quad ;;for\ highest\ order\ N-grams  $$
$$f(a_n|a_2\cdots a_{n-1}) =  \frac{c(a_n|*\cdots a_{n-1}) - D_1 } {n(\*a_2\cdots a_{n-2}\*) }\quad ;;for\ lower\ order\ N-grams $$ 

where the $n(|*a_2\cdots a_{n-1}a_n) notation represents the number of unique N-grams that match a given pattern with (*) used as a wildcard for a single word. 
...
$$p(a_n|a_1\cdots a_{n-1}) = (c(a_n|a_1\cdots a_{n-1}) > 0)\ ?\ f(a_n|a_1\cdots a_{n-1}) : bow(a_1\cdots a_{n-1}) * p(a_2\cdots a_{n})\quad  Eqn.2  $$
$$bow(a_{n-1}|a_1\cdots a_{n-2})  = \frac{1-\sum_{Z_1}f(a_n|a_1\cdots a_{n-1})} {1-\sum_{Z_1}f(a_2\cdots a_{n})}\quad   Eqn.3  $$

The option -interpolate is used to create the interpolated versions of -kndiscount and -ukndiscount. In this case we have:  
$$p(a_n|a_1\cdots a_{n-1}) = g(a_n|a_1\cdots a_{n-1}) + bow(a_1\cdots a_{n-1}) * p(a_2\cdots a_{n})\quad Eqn.4  $$ 

Let $Z_1$ be the set ${z: c(a_1\cdots a_{n}) > 0}$ . For highest order N-grams we have:  
...
Here $n(a_1\cdots a_{n-2}\*)$ represents the number of unique words following the context $(a_{n-1}|a_1\cdots a_{n-2})$  in the training data.   
...
$$p(a_n|a_1\cdots a_{n-1}) = (c(a_n|a_1\cdots a_{n-1}) > 0) ? f(a_n|a_1\cdots a_{n-1}) : bow(a_1\cdots a_{n-1}) * p(a_2\cdots a_{n})\quad;  Eqn.2   $$
$$bow(a_1\cdots a_{n-1})  = \frac{1-\sum_{Z_1}f(a_n|a_1\cdots a_{n-1})} {1-\sum_{Z_1}f(a_2\cdots a_{n})}\quad;  Eqn.3  $$
